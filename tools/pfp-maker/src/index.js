...
function resetEditor() {
	...
	editor.flagId = 0; // Rainbow overlay
}

// --- editing
// generate overlay buttons
for (var i = 0; i < presetOverlays.length; i++) {
	// create elements
	...
	presetOverlays[i].obj.DrawFlagOnCanvas(canv);

	const title = document.createElement("span");
	title.innerHTML = presetOverlays[i].name + "<br>";
...
// generate other overlay upload button
...
function setFlagMode(val) {
	...
	if (val) {
		...
		flagModeToggleHandle.style.left =
			...
			togglerectborder * 2 +
			"px";
		flagModeText.innerHTML = "overlay mode: BACKGROUND";
	}
	// toggled off
	else {
		flagModeToggle.style.backgroundColor = "#333333";
		flagModeToggleHandle.style.left = "0";
		flagModeText.innerHTML = "overlay mode: RING";
	}
